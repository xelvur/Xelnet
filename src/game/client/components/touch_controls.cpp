...
// simulate-key удалён (убрали SDL2 зависимость)
...
void CTouchControls::CTouchButtonBehavior::SetActive(const IInput::CTouchFingerState &FingerState)
{
	...
	{
		// FIX CRASH: SDL иногда меняет finger ID при скольжении пальца по краю кнопки.
		// Раньше здесь был dbg_assert_failed() — краш в debug сборке, UB в release.
		// Теперь просто переключаемся на новый палец без повторной активации.
		m_Finger = FingerState.m_Finger;
		m_ActivePosition = Position;
		m_AccumulatedDelta += Delta;
		OnUpdate();
...
{
	// FIX STRANGE HOLD BEHAVIOR: автоповтор отключён.
	// Раньше: после 250мс зажатия команда начинала слаться 15 раз/сек повторно.
	// Для игровых команд (jump, hook, fire) это вызывало дёрганье и двойные срабатывания.
	// Команда посылается один раз при нажатии (OnActivate) и при отпускании (OnDeactivate).
}
...
// simulate-key реализация удалена
...
void CTouchControls::UpdateButtonsGame(const std::vector<IInput::CTouchFingerState> &vTouchFingerStates)
{
	...
	for(CTouchButton &TouchButton : m_vTouchButtons)
	{
		...
		// FIX CRASH ON RECONNECT: при смене состояния (reconnect, map change) палец мог
		// исчезнуть из списка пока кнопка ещё помечена active. Вместо краша — деактивируем кнопку.
		if(ActiveFinger == vRemainingTouchFingerStates.end())
		{
			TouchButton.m_pBehavior->SetInactive(false);
			continue;
