...
void CMenus::RenderSettingsTClientSettings(CUIRect MainView)
{
	...
	{
		// Выпадающий список пресетов соотношения сторон (как Hammer Mode выше)
		static std::vector<const char *> s_StretchDropDownNames;
		s_StretchDropDownNames = {
			TCLocalize("4:3 (Классика)", "Stretch"),
			TCLocalize("16:10", "Stretch"),
			TCLocalize("5:4", "Stretch"),
			TCLocalize("3:2", "Stretch"),
			TCLocalize("1:1 (Квадрат)", "Stretch"),
		};
		static CUi::SDropDownState s_StretchDropDownState;
		static CScrollRegion s_StretchDropDownScrollRegion;
		s_StretchDropDownState.m_SelectionPopupContext.m_pScrollRegion = &s_StretchDropDownScrollRegion;

		// Определяем текущий выбранный пресет по значениям конфига
		int StretchPreset = 0; // default 4:3
		if(g_Config.m_TcStretchWidth == 4 && g_Config.m_TcStretchHeight == 3) StretchPreset = 0;
		else if(g_Config.m_TcStretchWidth == 8 && g_Config.m_TcStretchHeight == 5) StretchPreset = 1;
		else if(g_Config.m_TcStretchWidth == 5 && g_Config.m_TcStretchHeight == 4) StretchPreset = 2;
		else if(g_Config.m_TcStretchWidth == 3 && g_Config.m_TcStretchHeight == 2) StretchPreset = 3;
		else if(g_Config.m_TcStretchWidth == 1 && g_Config.m_TcStretchHeight == 1) StretchPreset = 4;

		CUIRect DropDownRow;
		Column.HSplitTop(LineSize, &DropDownRow, &Column);
		CUIRect StretchLabel;
		DropDownRow.VSplitLeft(120.0f, &StretchLabel, &DropDownRow);
		Ui()->DoLabel(&StretchLabel, TCLocalize("Aspect ratio: "), FontSize, TEXTALIGN_ML);

		int NewPreset = Ui()->DoDropDown(&DropDownRow, StretchPreset, s_StretchDropDownNames.data(), s_StretchDropDownNames.size(), s_StretchDropDownState);
		if(NewPreset != StretchPreset)
		{
			switch(NewPreset)
			{
			case 0: g_Config.m_TcStretchWidth = 4; g_Config.m_TcStretchHeight = 3; break; // 4:3
			case 1: g_Config.m_TcStretchWidth = 8; g_Config.m_TcStretchHeight = 5; break; // 16:10
			case 2: g_Config.m_TcStretchWidth = 5; g_Config.m_TcStretchHeight = 4; break; // 5:4
			case 3: g_Config.m_TcStretchWidth = 3; g_Config.m_TcStretchHeight = 2; break; // 3:2
			case 4: g_Config.m_TcStretchWidth = 1; g_Config.m_TcStretchHeight = 1; break; // 1:1
			}
		}
	}
	Column.HSplitTop(LineSize, &Button, &Column);
