...
{
	// FIX CRASH ON RECONNECT: SetAspectOverride идёт ПЕРВЫМ, до Clear.
	// OnNewSnapshot() читает ScreenAspect() который возвращает m_AspectOverride.
	// Если override ставить после Clear — первый тик после reconnect получает неверный aspect.
	// FIX DIV/ZERO: проверяем TcStretchHeight > 0 перед делением.
...
void CGameClient::OnNewSnapshot()
{
	...
	// send show distance
	// FIX: float сравнение через epsilon вместо != (нестабильное при stretch override)
